...
function SuccessContent() {
    ...
    return (
      <div className="min-h-screen bg-[var(--bg-soft)] flex items-center justify-center">
        <div className="text-center">
          <svg className="animate-spin w-8 h-8 text-[var(--accent)] mx-auto" fill="none" viewBox="0 0 24 24">
    ...
    return (
      <div className="min-h-screen bg-[var(--bg-soft)] flex items-center justify-center px-4">
        <div className="text-center">
          ...
          <a href="/" className="mt-4 inline-block text-sm text-[var(--accent)] hover:underline">Volver a la tienda</a>
        </div>
      </div>
    );
  }
  ...
  const config = {
    ...
    rejected: { msg: 'Pago rechazado', sub: 'Intentá con otro medio de pago', color: 'text-[var(--danger)]', bg: 'bg-[var(--danger-soft)]', icon: 'M9.75 9.75l4.5 4.5m0-4.5l-4.5 4.5M21 12a9 9 0 11-18 0 9 9 0 0118 0z' },
  };
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-soft)] flex items-center justify-center px-4">
      <div className="bg-white border border-[var(--border)] rounded-2xl p-6 sm:p-8 max-w-md w-full shadow-sm">
        ...
        <a href="/tienda" className="mt-3 w-full block text-center py-2.5 rounded-full bg-[var(--accent)] text-white font-semibold text-sm hover:bg-[var(--accent-hover)] transition-colors">
          Volver a la tienda
        </a>
        ...
        {order.customerDni && (
          <p className="mt-3 text-center text-xs text-[var(--text-muted)]">
            También podés buscar tu pedido con tu DNI: <span className="font-mono font-semibold text-[var(--text)]">{order.customerDni}</span>
          </p>
        )}
      </div>
    </div>
